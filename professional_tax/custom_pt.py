...
@frappe.whitelist()
def run_salary_slip_generation(employees_json, args_json):
    import json
    ...
    for emp_id in employees:
        try:
            ...
            if not salary_structure:
                frappe.log_error(f"No salary structure assigned to {emp_id}")
                continue

            salary_slip = frappe.new_doc("Salary Slip")
            salary_slip.update(args)
            salary_slip.employee = emp_id
            salary_slip.insert()
            ...
            if state and gross_pay:
                state_doc = frappe.get_doc("State", state)
                for row in state_doc.formula:
                    if row.component == "Professional Tax" and row.formula:
                        try:
                            pt_amount = safe_eval(
                                row.formula,
                                {
                            ...
                            frappe.log_error(f"Error in PT formula for {emp_id}: {e}")
            ...
            frappe.log_error(f"Failed to process salary slip for {emp_id}: {e}")

    return "Salary Slips created with PT if applicable."
...
# def create_salary_slips_for_employees(employees, args, publish_progress=False):
#     for i, emp_id in enumerate(employees):
#         try:
#             employee = frappe.get_cached_doc("Employee", emp_id)
#             custom_state = employee.custom_state

#             salary_structure = frappe.get_cached_value(
#                 "Salary Structure Assignment",
#                 {"employee": emp_id},
#                 "salary_structure"
#             )
...
#             if not salary_structure:
#                 frappe.log_error(f"No salary structure assigned for employee {emp_id}")
#                 continue
...
#             if custom_state:
#                 try:
#                     state_doc = frappe.get_doc("State", custom_state)
#                     base = employee.base or 0

#                     for row in state_doc.formula:
#                         if row.component == "Professional Tax" and row.formula:
#                             pt_amount = safe_eval(row.formula, {"base": base})
#                             break
#                 except Exception as e:
#                     frappe.log_error(f"Error calculating PT for employee {emp_id}: {e}")
...
#                     percent=(i + 1) * 100 // len(employees),
#                     title=f"Processing {employee.name}"
#                 )

#         except Exception as e:
#             frappe.log_error(f"Error processing salary slip for employee {emp_id}: {e}")
...
def create_salary_slips_for_employees(employees, args, publish_progress=False):
    for emp_id in employees:
        ...
        # Initialize PT amount
        pt_amount = 0

        if salary_structure and custom_state:
            state_doc = frappe.get_doc("State", custom_state)

            # Get formula for Professional Tax from the child table
            pt_formula_row = next((row for row in state_doc.formula if row.component == "Professional Tax"), None)

            if pt_formula_row and pt_formula_row.formula:
                try:
                    # Fetch base salary from employee or assign default
                    base = employee.base or 0

                    # Safe eval recommended for formulas
                    pt_amount = safe_eval(pt_formula_row.formula, {"base": base})

                    frappe.logger().info(f"Calculated PT for {emp_id}: {pt_amount}")
                except Exception as e:
                    frappe.log_error(f"PT Calculation Error for {emp_id}: {e}", "PT Evaluation Error")
